...
def test_f14_tags(page, qapi):
    F = Soft("F14", page, S["rec"])
    goto(page, "/transactions.html?range=all", wait_sel="#tx-body tr[data-id]", soft=F, label="transactions for tags")
    page.wait_for_timeout(400)
    rows = page.locator("#tx-body tr[data-id]")
    tid = int(rows.first.get_attribute("data-id"))
    # tag from the drawer with inline create
    rows.first.locator(".col-merchant").click()
    page.wait_for_selector(".drawer [data-dact='tags']", timeout=8000)
    page.locator(".drawer [data-dact='tags']").click()
    page.wait_for_selector(".tag-picker input", timeout=4000)
    page.locator(".tag-picker input").type("QA Trip")
    page.wait_for_timeout(200)
    page.locator(".tag-picker [data-create]").click()
    page.wait_for_timeout(900)
    F.check(any(t["name"] == "QA Trip" for t in qapi.get("/api/tags")), "tag created from the picker")
    tag = next(t for t in qapi.get("/api/tags") if t["name"] == "QA Trip")
    F.check(qapi.get(f"/api/transactions/{tid}")["tag_ids"] == [tag["id"]], "tag attached to the transaction")
    page.keyboard.press("Escape")
    page.wait_for_timeout(200)
    F.check(page.locator(f".drawer .tagchip[data-tag-id='{tag['id']}']").count() == 1, "drawer shows the tag chip")
    page.keyboard.press("Escape")
    page.wait_for_selector(".drawer", state="detached", timeout=5000)
    F.check(page.locator(f"#tx-body tr[data-id='{tid}'] .tagchip").count() == 1, "row shows the tag chip")
    # filter by tag
    page.locator("#f-tags").click()
    page.wait_for_selector(".popover [data-v]", timeout=4000)
    page.locator(f".popover [data-v='{tag['id']}']").click()
    page.wait_for_url(re.compile(rf"tag={tag['id']}"), timeout=5000)
    page.keyboard.press("Escape")
    page.wait_for_timeout(600)
    F.check(page.locator("#tx-body tr[data-id]").count() == 1 and "QA Trip" in page.locator("#f-tags").inner_text(), "tag filter narrows the list and labels the button")
    page.locator("#f-clear").click()
    page.wait_for_timeout(700)
    # bulk tag two rows with undo
    rows = page.locator("#tx-body tr[data-id]")
    ids = [int(rows.nth(i).get_attribute("data-id")) for i in (1, 2)]
    rows.nth(1).locator("input[data-select]").check()
    rows.nth(2).locator("input[data-select]").check()
    page.locator("#tx-bulk [data-bulk='tag']").click()
    page.wait_for_selector(".tag-picker [data-tag]", timeout=4000)
    page.locator(f".tag-picker [data-tag='{tag['id']}']").click()
    t = toast(page, "Tagged")
    F.check("Undo" in t and all(tag["id"] in qapi.get(f"/api/transactions/{i}")["tag_ids"] for i in ids), "bulk tag applied with undo")
    page.keyboard.press("Escape")
    click_undo(page)
    toast(page, "Undone")
    page.wait_for_timeout(500)
    F.check(all(tag["id"] not in qapi.get(f"/api/transactions/{i}")["tag_ids"] for i in ids), "undo removed the bulk tags")
    dismiss_toasts(page)
    page.keyboard.press("Escape")
    # manage on the Categories page
    goto(page, "/categories.html", wait_sel="#tags-list .tag-row", soft=F, label="categories with tags")
    page.locator(f"#tags-list [data-tag='{tag['id']}'] [data-tact='rename']").click()
    page.wait_for_selector("#tag-name", timeout=4000)
    page.fill("#tag-name", "QA Holiday")
    page.locator(".modal-foot .btn-primary").click()
    toast(page, "renamed")
    page.wait_for_timeout(500)
    F.check(next(t for t in qapi.get("/api/tags") if t["id"] == tag["id"])["name"] == "QA Holiday", "rename persisted")
    page.locator(f"#tags-list [data-tag='{tag['id']}'] [data-tact='color']").click()
    page.wait_for_selector(".popover [data-color='c7']", timeout=4000)
    page.locator(".popover [data-color='c7']").click()
    page.wait_for_timeout(600)
    F.check(next(t for t in qapi.get("/api/tags") if t["id"] == tag["id"])["color"] == "c7", "colour persisted")
    page.locator(f"#tags-list [data-tag='{tag['id']}'] [data-tact='delete']").click()
    page.wait_for_selector(".modal", timeout=4000)
    page.locator(".modal-foot .btn-danger-solid, .modal-foot .btn-primary").last.click()
    toast(page, "Tag deleted")
    page.wait_for_timeout(500)
    F.check(not any(t["id"] == tag["id"] for t in qapi.get("/api/tags")), "tag deleted")
    F.check(qapi.get(f"/api/transactions/{tid}")["tag_ids"] == [], "deleting the tag detached it from the transaction")
    F.finish()
