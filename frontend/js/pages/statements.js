...
function statementRow(s) {
  ...
  return `<tr data-id="${s.id}" class="${isClickable ? 'is-clickable' : ''}">
    <td class="col-file"><div class="st-file"><span class="st-file-icon ${esc(s.file_kind)}">${icon(KIND_ICON[s.file_kind] || 'file')}</span>
      <div class="merchant">${href ? `<a class="st-file-name row-link" href="${href}" data-tip="${esc(s.original_filename)}"><span class="truncate">${esc(s.original_filename)}</span></a>` : `<span class="st-file-name" data-tip="${esc(s.original_filename)}">${esc(s.original_filename)}</span>`}<span class="st-file-meta">${esc(String(s.file_kind || '').toUpperCase())} · ${fmtBytes(s.file_size)}${s.ocr_applied ? ' · OCR' : ''}</span></div></div></td>
    ...
    <td class="col-status" data-label="Status"><span class="st-status"><span class="badge ${st.cls}" aria-label="Status: ${esc(st.label)}${st.spin ? ', in progress' : ''}"${st.spin ? ' aria-busy="true"' : ''}>${st.spin ? '<span class="spinner" aria-hidden="true"></span>' : ''}${esc(st.label)}</span>${s.status === 'error' && s.error_message ? `<span class="text-3 fs-xs truncate" style="max-width:220px" data-tip="${esc(s.error_message)}">${esc(s.error_message)}</span>` : ''}</span></td>
    <td class="right col-rows" data-label="Rows">${rows}</td>
    <td class="col-uploaded text-3" data-label="Uploaded" data-tip="${esc(fmtDateTime(s.created_at))}">${fmtRelative(s.created_at)}</td>
