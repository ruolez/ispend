...
function statementRow(s) {
  ...
  return `<tr data-id="${s.id}" class="${isClickable ? 'is-clickable' : ''}">
    <td class="col-file"><div class="st-file"><span class="st-file-icon ${esc(s.file_kind)}">${icon(KIND_ICON[s.file_kind] || 'file')}</span>
      <div class="merchant">${href ? `<a class="st-file-name row-link" href="${href}" data-tip="${esc(s.original_filename)}">${esc(s.original_filename)}</a>` : `<span class="st-file-name" data-tip="${esc(s.original_filename)}">${esc(s.original_filename)}</span>`}<span class="st-file-meta">${esc(String(s.file_kind || '').toUpperCase())} · ${fmtBytes(s.file_size)}${s.ocr_applied ? ' · OCR' : ''}</span></div></div></td>
