...
/* The stacked monthly chart ends on the month the range picker points at (this month for open-ended presets). */
function rangeEndMonth() {
  const d = rangeDates(state.range);
  if (!d || !d.to) return null;
  const ym = String(d.to).slice(0, 7);
  return ym === currentMonth() ? null : ym;
}
...
function renderMerchants(res, cats) {
  ...
  tb.innerHTML = rows.map((m, i) => { const c = cats.get(m.category_id); const color = c ? (c.color || c.parent_color) : null; return `<tr class="is-clickable" data-href="/transactions.html${toQuery({ q: m.merchant_name, range: 'all' })}">
    <td><a class="row-link" href="/transactions.html${toQuery({ q: m.merchant_name, range: 'all' })}"><span class="cat-cell"><span class="text-4 fs-xs num" style="width:18px">${i + 1}</span><span class="name">${esc(m.merchant_name)}</span></span></a></td>
...
async function loadCompare(force) {
  ...
  if (!host.querySelector('#ch-compare')) host.innerHTML = `<div class="compare-row mb-4">
      <div class="cmp-pick"><span class="cmp-label" id="cmp-month-l">This period</span><div class="row gap-1"><button type="button" class="btn btn-icon btn-secondary" data-act="cmp-prev" aria-label="Previous month">${icon('chevron-left')}</button><button type="button" class="btn btn-secondary cmp-month-btn" id="cmp-month" aria-haspopup="dialog"></button><button type="button" class="btn btn-icon btn-secondary" data-act="cmp-next" aria-label="Next month">${icon('chevron-right')}</button></div></div>
      ...
      <div class="cmp-pick"><span class="cmp-label" id="cmp-vs-l">Compare with</span><div class="row gap-1"><button type="button" class="btn btn-icon btn-secondary" data-act="vs-prev" aria-label="Earlier month">${icon('chevron-left')}</button><button type="button" class="btn btn-secondary cmp-month-btn" id="cmp-vs" aria-haspopup="dialog"></button><button type="button" class="btn btn-icon btn-secondary" data-act="vs-next" aria-label="Later month">${icon('chevron-right')}</button></div></div>
  ...
  // two month buttons with the shared month picker (the native month input differs on every phone)
  const paintMonthBtn = (id, ym) => { const b = $(id); b.dataset.value = ym; b.setAttribute('aria-label', `${id === '#cmp-month' ? 'This period' : 'Compare with'}: ${fmtMonth(ym, { long: true })}`); b.innerHTML = `${icon('calendar', 'ico-sm')}<span class="label">${esc(fmtMonth(ym, { long: true }))}</span>${icon('chevron-down', 'ico-sm')}`; };
  paintMonthBtn('#cmp-month', state.month);
  paintMonthBtn('#cmp-vs', state.vs || prevMonthOf(state.month));
  $('#cmp-month').onclick = (e) => monthPicker({ anchor: e.currentTarget, value: state.month, max: currentMonth(), onPick: (ym) => { state.month = ym; sync(); loadCompare(true); } });
  $('#cmp-vs').onclick = (e) => monthPicker({ anchor: e.currentTarget, value: state.vs || prevMonthOf(state.month), max: currentMonth(), onPick: (ym) => { state.vs = ym; sync(); loadCompare(true); } });
  const seq = ++state.seq;
...
function renderCompare(data) {
  ...
  const chart = charts.makeChart($('#ch-compare'), (th) => ({
    ...
    options: { ...charts.barOptions(th, { currency: cur }), scales: { x: { grid: { display: false }, border: { display: false }, ticks: { maxRotation: ui.isPhone() ? 0 : 45, autoSkip: ui.isPhone(), callback: (v, i) => { const l = cats[i].name; const max = ui.isPhone() ? 9 : 18; return l.length > max ? l.slice(0, max - 1) + '…' : l; } } }, y: { beginAtZero: true, border: { display: false }, ticks: charts.currencyTicks(cur) } } },
  }));
