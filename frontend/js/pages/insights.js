...
function renderRecurring() {
  ...
  host.innerHTML = `<div class="tbl-wrap tbl-wrap--flush"><table class="tbl rec-table tbl--cards"><thead><tr><th class="col-merchant">Merchant</th><th class="col-cat">Category</th><th class="col-cadence">Cadence</th><th class="right col-amt">Amount</th><th class="col-last">Last</th><th class="right col-next">Next</th><th class="col-actions"><span class="sr-only">Actions</span></th></tr></thead><tbody>
    ${rows.map((r) => { const c = state.cats.get(r.category_id); const n = r.days_until_next; return `<tr data-key="${esc(r.merchant_key)}" class="${r.is_active ? '' : 'text-3'}">
      ...
      <td class="col-cat" data-label="Category">${c ? `<span class="catchip"><i class="dot" style="--c:var(--${esc(c.color || c.parent_color || 'c1')})"></i><span class="catchip-label">${esc(c.name)}</span></span>` : '<span class="text-4">—</span>'}</td>
      <td class="col-cadence"><span class="badge badge-neutral cadence" aria-label="Cadence: ${esc(r.cadence)}">${esc(r.cadence)}</span></td>
      <td class="right col-amt" data-label="Amount"><div class="rec-amt"><span class="num fw-500">${fmtMoney(r.median_amount, cur)}</span>${r.amount_kind === 'variable' ? '<span class="badge badge-warning" aria-label="Amount varies between charges">varies</span>' : ''}</div></td>
      <td class="col-last text-3">${fmtDate(r.last_date, { year: true })}</td>
      <td class="right col-next" data-label="Next"><div class="rec-next">${r.is_active ? `<span class="when ${n != null && n < 0 ? 'is-over' : (n != null && n <= 3 ? 'is-soon' : '')}">${esc(daysLabel(n))}</span><span class="date">${fmtDate(r.next_expected)}</span>` : `<span class="date">Last ${fmtDate(r.last_date)}</span>`}</div></td>
