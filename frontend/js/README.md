...
- **Sheets.** On phones (≤768px) every `ui.modal` docks to the bottom as a sheet (grab handle, swipe down to dismiss, sticky footer with equal-width buttons, no field autofocus on touch so the keyboard doesn't cover it); pass `sheet: false` to opt out. Every `ui.popover` (menus, multiFilter, all pickers) becomes a sheet over a `.sheet-backdrop`; input autosuggests pass `noSheet: true`. `ui.sheet(opts)` = a modal sheet that the browser/OS Back gesture also closes — close it *before* calling `setQs()` (a replaceState drops its history mark). `ui.dragToDismiss(panel, fn, {zone})` is the gesture on its own. `ui.isPhone()` / `ui.isCoarse()`.
- **Gestures** (always doubling a visible control): `ui.longPress(root, selector, fn(el))` (450ms, the following click is swallowed) and `ui.swipe(root, selector, { left: {label, icon, cls:'is-ok'|'is-muted', run(el)}, right: {…} })` (35% of the row or a flick commits; vertical scroll wins).
- **Tooltips on touch.** Use `data-tip`, never `title=""` (test_mobile rejects it): tapping a non-control element with `data-tip` shows the bubble for 2.5s.
- **Month picker.** `monthPicker({ anchor, value:'YYYY-MM', min, max, onPick(ym), extra:[{label, icon, onClick}] })` — year stepper + month grid; the one month control (Dashboard, Budgets, Insights).
- **Charts.** Wrap a navigating `onClick` in `charts.tapToDrill(fn)`: a mouse click navigates at once, a touch shows the tooltip ("Tap again to open") and the second tap on the same bar navigates. Phones get at most 6 axis ticks.
- **Floatbar.** On phones `.floatbar` spans the width above the bottom nav and scrolls sideways instead of overflowing; toasts lift above it while it is visible.
- **Phone shell.** Bottom tabs are Home · Transactions · Review · Budgets · More; More opens a sheet with every other page, pinned views, theme, install and account. There is no hamburger or theme toggle on phones. Mark a page's main header button `data-primary-action` and it is mirrored as a round "+" in the topbar (the header copy hides); mark secondary header buttons `data-overflow` and they fold into a "⋯" beside the title. The page H1 is the large title; the topbar shows its own title only after the H1 scrolls under it. `.phone-only` shows an element at ≤768px only.
- **Transactions on phones.** Sticky search + Filters button over a sideways chip strip (`.tx-chips`, `display: contents` on desktop); `openFilterSheet()` buffers every filter plus the sort and counts matches with a `limit=1` call before applying. Rows become [category disc] merchant · amount / category · account, grouped under sticky day headers (`tr.tx-day`, phone-only, rebuilt by `paintDays()` after every render). Long-press or ⋯ › Select transactions enters selection mode (`tx.selectMode`, `#tx-table.is-selecting`): taps toggle rows, the bar holds Categorize · More · ✕. Swipe left = Categorize, right = Exclude/Include (undoable).
- **List rows.** Add `tbl--list` next to `tbl--cards` for record tables (statements, accounts, admin users): on phones the first cell is the title and the other cells run on as one dotted meta line instead of LABEL/value stacks; `data-unit=" transactions"` appends a unit to a bare number, `.hide-mobile` cells drop out. Keep plain `tbl--cards` where the labels carry meaning (report comparisons).
